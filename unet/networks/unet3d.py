...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_batch_norm=True, conv_kernel_size=3):
        super().__init__()
        ...
        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel_size=conv_kernel_size, padding=self.padding)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv3d(out_channels, out_channels, kernel_size=conv_kernel_size, padding=self.padding)
...
class Encoder(nn.Module):
    def __init__(self, input_channels: int, feature_map: list[int], use_batch_norm: bool, dropout_probability: float, conv_kernel_size: int):
        super().__init__()
        ...
        self.encoder_blocks = nn.ModuleList(
            [ConvBlock(self.encoder_features[i], self.encoder_features[i+1], use_batch_norm, conv_kernel_size=conv_kernel_size) for i in range(len(self.encoder_features) - 1)])
...
class Decoder(nn.Module):
    def __init__(self, feature_map: list[int], use_batch_norm: bool, dropout_probability: float, conv_kernel_size: int):
        super().__init__()
        self.dropout_probability = dropout_probability
        self.feature_map = feature_map
        self.up_conv = nn.ModuleList(
            [nn.ConvTranspose3d(feature_map[i], feature_map[i+1], kernel_size=2, stride=2) for i in range(len(feature_map) - 1)]
            )
        self.decoder_block = nn.ModuleList(
            [ConvBlock(feature_map[i], feature_map[i+1], use_batch_norm, conv_kernel_size=conv_kernel_size) for i in range(len(feature_map) - 1)]
        )
        self.dropout = nn.Dropout3d(p=self.dropout_probability)
...
class UNet3D(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.feature_map = calculate_feature_channels(initial_feature, feature_multiplier, network_depth)
        self.encoder = Encoder(input_channels, self.feature_map, use_batch_norm=use_batch_norm, dropout_probability=dropout_probability, conv_kernel_size=conv_kernel_size)
        self.decoder = Decoder(self.feature_map[::-1], use_batch_norm=use_batch_norm, dropout_probability=dropout_probability, conv_kernel_size=conv_kernel_size)

...
def dice_coef(prediction, target):
    assert torch.is_tensor(prediction) and torch.is_tensor(target), "Expected torch tensor input"
...
def calculate_rand_error(prediction, target, thresholds=[0.4]):
    """Calculate adapted rand across flattened arrays and given threshold intervals"""
    if torch.is_tensor(target):
        # prediction, target = prediction.detach().numpy(), target.detach().numpy()
        prediction, target = prediction.detach().cpu().numpy(), target.detach().cpu().numpy()
    else:
        # Function called outside of training/validation, so calculate edges
        target = skimage.segmentation.find_boundaries(target, mode="thick", connectivity=2)
    ...
    if np.all(target == target[0]):
        # Only a single label, so the best Rand score is 0
        return np.array([0] * len(thresholds))

    rand_error = np.array([])
    for th in thresholds:
        pred_th = prediction > th
        rand_error = np.append(rand_error, skimage.metrics.adapted_rand_error(target, pred_th)[0])
    return rand_error